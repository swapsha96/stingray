...
class TestRebinDataLog(object):
    ...
    def setup_class(cls):
        ...
        cls.true_bin_edges = np.array([0.5,
                                       1.5,
                                       2.6000000000000001,
                                       3.81,
                                       5.141,
    ...
    def test_binning_works_correctly(self):
        binx, _, _, _ = utils.rebin_data_log(
            self.x, self.y, self.f, y_err=self.y_err, dx=self.dx)
    ...
    def test_bin_values_are_correct(self):
        _, biny, _, _ = utils.rebin_data_log(
            self.x, self.y, self.f, y_err=self.y_err, dx=self.dx)
    ...
    def test_method_works_on_complex_numbers(self):
        ...
        _, biny, _, _ = utils.rebin_data_log(
            self.x, y, self.f, y_err=yerr, dx=self.dx)
...
class TestUtils(object):
    ...
    def test_get_random_state(self):
        ...
        assert np.all(utils.get_random_state(lue).randn(
...
class TestCreateWindow(object):
    ...
    def test_flat_top_window(self):
        result = np.array([8.67361738e-17, -
                           2.62000000e-01, 4.63600000e+00, -
                           2.62000000e-01, 8.67361738e-17])
